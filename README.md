...
* В папке SERVER лежит исполняемый файл сервера, в папке INSTALLER лежит установщик для клиента. Для их запуска не требуется предустановленая java и python. СЕРВЕР МОЖЕТ ИНОГДА ЗАГРУЖАТЬ ГРАФИЧЕСКИЙ ИНТЕРФЕЙС НЕКОТОРОЕ ВРЕМЯ.
